import type { LLMRouterModel } from '../types'

/**
 * Strip Vertex-style version pinning suffixes that have no place in a
 * display name. LLMRouter exposes Anthropic-on-Vertex models with ids
 ...
 */
export function categorizeModel(model: LLMRouterModel): 'chat' | 'embedding' | 'image' | 'audio' | 'unknown' {
  if (model.mode) {
...
/**
 * Format a LLMRouter model ID into a human readable display name.
 *
 * Pre-processing:
 ...
 */
export function formatModelName(model: LLMRouterModel): string {
  const { id } = model
